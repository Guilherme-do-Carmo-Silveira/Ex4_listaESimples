...
public class Lista {
	...
	private Boolean Verifica(int valor) throws Exception {
		int tamanho = size();
		boolean status = false;
		for (int i = 0; i < tamanho; i++) {
			if (valor == get(i)) {
				status = true;
				throw new Exception("O valor já existe na lista");
			}
		}
		return status;
	}
